...
def select_tcp_pose_key(episode_data: dict, expected_keys: Optional[set] = None) -> Optional[str]:
    candidates = [k for k in episode_data.keys() if "robot_tcp_pose" in k.lower() and "wrt" not in k.lower()]
    if expected_keys:
        filtered = [k for k in candidates if k in expected_keys]
        if filtered:
            candidates = filtered
    ...
    if not candidates:
        return None
    if "left_robot_tcp_pose" in candidates:
        return "left_robot_tcp_pose"
    return candidates[0]
...
def predict_actions_chunk_at_index(
    ...
    def _to_batch(x: torch.Tensor):
        x = x.unsqueeze(0).to(device)
        bsz = max(1, int(repeat_batch_size))
        if bsz > 1:
            x = x.repeat(*([bsz] + [1] * (x.ndim - 1)))
        return x

    obs = dict_apply(obs, _to_batch)
    with torch.no_grad():
        result = policy.predict_action(obs)
    pred_full = result["action_pred"].detach().cpu().numpy()  # [B, T, D]
    pred_seq = pred_full[:, :n_action_steps]
    return pred_seq
...
def actions_to_pos_traj(action_seq: np.ndarray, base_pos: np.ndarray, relative: bool) -> np.ndarray:
    if action_seq is None or len(action_seq) == 0:
        return np.empty((0, 3), dtype=np.float32)
    xyz = np.asarray(action_seq)[..., :3]
    if relative:
        return base_pos.reshape(1, 3) + np.cumsum(xyz, axis=0)
    return xyz


def plot_3d_repeated_trajectories(
    episode_idx: int,
    ...
    if pred_chunks is None or pred_chunks.size == 0:
        return

    out_dir = pathlib.Path(output_dir).expanduser().resolve()
    out_dir.mkdir(parents=True, exist_ok=True)

    B, T, _ = pred_chunks.shape
    gt_chunk = gt_actions[idx:min(idx + T + 1, len(gt_actions))]
    gt_action = absolute_actions_to_relative_actions(
                    gt_chunk, base_absolute_action=gt_chunk[0],
    ...
    for b in range(B):
        traj = actions_to_pos_traj(pred_chunks[b], base_pos, True)
        if len(traj) == 0:
            continue
        ax.plot(traj[:, 0], traj[:, 1], traj[:, 2], linewidth=1.8, alpha=0.8, label=f"pred_{b}")

    gt_traj = actions_to_pos_traj(gt_action, base_pos, True)
    if len(gt_traj) > 0:
        ax.plot(
            gt_traj[:, 0], gt_traj[:, 1], gt_traj[:, 2],
            color="black", linewidth=2.5, linestyle="--", label="gt"
        )

    ax.scatter([base_pos[0]], [base_pos[1]], [base_pos[2]], color="red", s=35, label="start")
    ax.set_title(f"Episode {episode_idx} - Repeated Inference 3D Position Trajectories")
    ax.set_xlabel("X")
    ax.set_ylabel("Y")
    ax.set_zlabel("Z")
    ...
    save_path = out_dir / f"episode_{episode_idx}_repeated_3d.png"
...
def main():
    ...
    parser.add_argument("--debug-episode", type=int, default=20, help="If set, only run this episode and save debug plot")
    parser.add_argument("--debug-dir", default="./debug_sign_accuracy", help="Output directory for debug plots")
    parser.add_argument("--repeat-batch-size", type=int, default=10, help="Repeat one observation into a batch for stochastic multi-sample inference")
    ...
    for ep_idx in range(start_ep, end_ep):
        ...
        pred_chunks = predict_actions_chunk_at_index(
            policy,
            episode_data,
            device,
        ...
        pred_chunk = pred_chunks[0] if pred_chunks.ndim == 3 and len(pred_chunks) > 0 else np.empty((0,))
        ...
        print(
            ...
            f"repeat_batch={max(1, int(args.repeat_batch_size))}"
        )
        episode_results.append((ep_idx, window[0], window[1], acc, correct, valid))

        if args.debug_episode is not None:
            tcp_key = select_tcp_pose_key(episode_data, expected_keys)
            if tcp_key is not None and len(episode_data[tcp_key]) > 0:
                base_idx = max(0, idx - 1)
                base_pos = np.asarray(episode_data[tcp_key][base_idx])[:3].astype(np.float32)
            else:
                base_pos = np.zeros(3, dtype=np.float32)

            print(pred_chunks[:,0,:3])
            print(pred_chunks[:,-1,:3])
            plot_3d_repeated_trajectories(
                ep_idx,
                pred_chunks,
                gt_actions,
                idx,
