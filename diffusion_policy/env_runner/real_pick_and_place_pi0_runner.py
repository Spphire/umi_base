...
class RealPickAndPlacePi0Runner:
    def __init__(self,
                 pi0_server_ip: str,
                 pi0_server_port: int,
        ...
        self.pi0_server_ip = pi0_server_ip
        self.pi0_server_port = pi0_server_port
        self.pi0_client = websocket_client_policy.WebsocketClientPolicy(host=pi0_server_ip, port=pi0_server_port)
            ...
            for episode_idx in tqdm.tqdm(range(0, self.eval_episodes),
                ...
                try:
                    while True:
                        ...
                        # convert to uint8 and reshape to (batch, h, w, c)
                        ...
                        observation = {
                            "observation/image": image_tools.convert_to_uint8(
                                image_tools.resize_with_pad(np_obs_dict['external_img'][-1], 224, 224)
                            ),
                            "observation/wrist_image": image_tools.convert_to_uint8(
                                image_tools.resize_with_pad(np_obs_dict['left_wrist_img'][-1], 224, 224)
                            ),
                            "observation/state": np.concatenate([
                                np_obs_dict['left_robot_tcp_pose'][-1] if 'left_robot_tcp_pose' in np_obs_dict else np.array([]),
                                np_obs_dict['left_robot_gripper_width'][-1] if 'left_robot_gripper_width' in np_obs_dict else np.array([])
                            ], axis=-1),
                            "prompt": "Grasp and place an object."
                        }
