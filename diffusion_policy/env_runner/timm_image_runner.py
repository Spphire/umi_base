...
from diffusion_policy.policy.vq_bet_image_policy import VqBetImagePolicy
...
class TimmImageRunner:
    def __init__(
        ...
        self.gripper_latency_step = (
            gripper_latency_step if gripper_latency_step is not None else latency_step
        )
        ...
        self.use_relative_tcp_obs_for_relative_action = (
            use_relative_tcp_obs_for_relative_action
        )
        ...
        if enable_video_recording:
            assert (
                isinstance(vcamera_server_ip, str)
                and isinstance(vcamera_server_port, int)
                or isinstance(vcamera_server_ip, ListConfig)
                and isinstance(vcamera_server_port, ListConfig)
            ), (
    ...
    def pre_process_obs(self, obs_dict: Dict) -> Tuple[Dict, Dict]:
        ...
        obs_dict.update(
            get_inter_gripper_actions(obs_dict, self.lowdim_keys, self.transforms)
        )
        for key in self.lowdim_keys:
    ...
    def post_process_action(self, action: np.ndarray) -> Tuple[np.ndarray, bool]:
        ...
        if self.env.data_processing_manager.use_6d_rotation:
            if action.shape[-1] == 4 or action.shape[-1] == 8:
                ...
                left_action_6d = np.concatenate(
                    [left_trans_batch, left_euler_batch], axis=1
                )  # (action_steps, 6)
                if action.shape[-1] == 8:
                    right_trans_batch = action[:, 3:6]  # (action_steps, 3)
                    right_euler_batch = np.zeros_like(right_trans_batch)
                    right_action_6d = np.concatenate(
                        [right_trans_batch, right_euler_batch], axis=1
                    )
                else:
                ...
                left_euler_batch = np.array(
                    [t3d.euler.mat2euler(rot_mat) for rot_mat in left_rot_mat_batch]
                )  # (action_steps, 3)
                left_trans_batch = action[:, :3]  # (action_steps, 3)
                left_action_6d = np.concatenate(
                    [left_trans_batch, left_euler_batch], axis=1
                ...
                if action.shape[-1] == 20:
                    ...
                    right_euler_batch = np.array(
                        [
                            t3d.euler.mat2euler(rot_mat)
    ...
    def action_command_thread(
        self, policy: Union[DiffusionUnetImagePolicy, VqBetImagePolicy], stop_event
    ):
        while not stop_event.is_set():
            ...
            if (
                tcp_step_action is None or gripper_step_action is None
            ):  # no action in the buffer => no movement.
                logger.debug(
                    f"None: {tcp_step_action is None}, {gripper_step_action is None}"
                )
                cur_time = time.time()
                precise_sleep(
                    max(0.0, self.control_interval_time - (cur_time - start_time))
                )
                logger.debug(
                    f"Step: {self.action_step_count}, control_interval_time: {self.control_interval_time}, "
            ...
            combined_action = np.concatenate(
                [tcp_step_action, gripper_step_action], axis=-1
            )
            ...
            step_action, is_bimanual = self.post_process_action(
                combined_action[np.newaxis, :]
            )
            step_action = step_action.squeeze(0)
            # [debug]
            step_action[6] *= self.debug_gripper_width_scale
    ...
    def run(self, policy: Union[DiffusionUnetImagePolicy, VqBetImagePolicy]):
        ...
        try:
            spin_thread = threading.Thread(
                target=self.spin_executor, args=(executor,), daemon=True
            )
            spin_thread.start()
            ...
            for episode_idx in tqdm.tqdm(
                ...
                if self.enable_video_recording:
                    video_path = os.path.join(
                        self.video_dir, f"episode_{episode_idx}.mp4"
                    )
                ...
                try:
                    while True:
                        ...
                        for i in imgs:
                            i = i.transpose(1, 2, 0)  # CHW to HWC
                            rr.log("left_wrist_img", entity=rr.Image(i))
                        ...
                        if self.use_relative_action:
                            base_absolute_action = np.concatenate(
                                [
                                    np_absolute_obs_dict["left_robot_tcp_pose"][-1]
                                    if "left_robot_tcp_pose" in np_absolute_obs_dict
                                    else np.array([]),
                                    np_absolute_obs_dict["right_robot_tcp_pose"][-1]
                                    if "right_robot_tcp_pose" in np_absolute_obs_dict
                                    else np.array([]),
                                ],
                                axis=-1,
                            )
                            action_all = relative_actions_to_absolute_actions(
                                action_all, base_absolute_action
                            )
                        ...
                        # [debug]
                        if step_count % self.tcp_action_update_interval == 0:
                            action_all_homo = pose_3d_9d_to_homo_matrix_batch(
                                action_all[:, :9]
                            )
                            visualize_pose_matrices(
                                action_all_homo,
                                f"predict_abs_action/frame_{step_count}",
                                show_coordinate_frame=True,
                            )

                        if self.action_interpolation_ratio > 1:
                            action_all = interpolate_actions_with_ratio(
                                action_all, self.action_interpolation_ratio
                            )
                        ...
                        if step_count % self.tcp_action_update_interval == 0:
                            ...
                            tcp_action_str = np.array2string(
                                tcp_action,
                            ...
                            if self.env.enable_exp_recording:
                                self.env.get_predicted_action(
                                    tcp_action, type="full_tcp"
                                )

                        if step_count % self.gripper_action_update_interval == 0:
                            if action_all.shape[-1] == 4:
                                gripper_action = action_all[
                                    self.gripper_latency_step :, 3:
                                ]
                            elif action_all.shape[-1] == 8:
                                gripper_action = action_all[
                                    self.gripper_latency_step :, 6:
                                ]
                            elif action_all.shape[-1] == 10:
                                gripper_action = action_all[
                                    self.gripper_latency_step :, 9:
                                ]
                            elif action_all.shape[-1] == 20:
                                gripper_action = action_all[
                                    self.gripper_latency_step :, 18:
                                ]
                            else:
                                raise NotImplementedError
                            # add to ensemble buffer
                            gripper_action_str = np.array2string(
                                gripper_action,
                                formatter={"float_kind": lambda x: f"{x:.3f}"},
                            )
                            logger.debug(
                                f"step {step_count} add gripper: {gripper_action_str}"
                            )
                            self.gripper_ensemble_buffer.add_action(
                                gripper_action, step_count
                            )
                        ...
                        precise_sleep(
                            max(
                                0.0,
                                self.inference_interval_time - (cur_time - start_time),
                            )
                        )
