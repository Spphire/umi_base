...
class SensorMessage(BaseModel):
    ...
    leftGripperCameraRGB1: np.ndarray = Field(
        default_factory=lambda: np.zeros((288, 352, 3), dtype=np.uint8))  # (H, W, 3) (r, g, b)
    leftGripperCameraMarker1: np.ndarray = Field(
        default_factory=lambda: np.zeros((300, 3), dtype=np.float32))  # (num_markers, 3)(x, y, z)
    ...
    leftGripperCameraMarkerOffset2: np.ndarray = Field(
        default_factory=lambda: np.zeros((300, 3), dtype=np.float32))  # (num_markers, 2)(x, y,z)
    rightGripperCameraRGB1: np.ndarray = Field(default_factory=lambda: np.zeros((288, 352, 3), dtype=np.uint8))  # (H, W, 3) (r, g, b)
    rightGripperCameraMarker1: np.ndarray = Field(
        default_factory=lambda: np.zeros((300, 3), dtype=np.float32))  # (num_markers, 3)(x, y, z)
    rightGripperCameraMarkerOffset1: np.ndarray = Field(
        default_factory=lambda: np.zeros((300, 3), dtype=np.float32))  # (num_markers, 3)(x, y, z)
    rightGripperCameraRGB2: np.ndarray = Field(default_factory=lambda: np.zeros((288, 352, 3), dtype=np.uint8))  # (H, W, 3) (r, g, b)
...
class SensorMode(Enum):
    single_arm_one_realsense_no_gelslim = auto()
    single_arm_two_realsense_no_gelslim = auto()
    single_arm_two_realsense_two_gelslim = auto()
    dual_arm_two_realsense_four_gelslim = auto()
