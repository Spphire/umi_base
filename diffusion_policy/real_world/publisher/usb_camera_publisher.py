...
from diffusion_policy.common.data_models import TactileSensorMessage, Arrow

import ComputePolygonArea
import os
...
class UsbCameraPublisher(Node):
    ...
    def extractMarker(self, img):
        ...
        if self.dimension == 3:
            points_arr = np.ascontiguousarray(MarkerCenter[:, :2], dtype=np.float64)
            MarkerCenter[:, 2] = ComputePolygonArea.get_contour_area(points_arr, num_threads=10)
