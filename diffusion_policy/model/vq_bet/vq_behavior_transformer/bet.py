...
import tqdm
import numpy as np
from diffusion_policy.model.vq_bet.vq_behavior_transformer.gpt import GPT
from diffusion_policy.model.vq_bet.vq_behavior_transformer.utils import MLP
from diffusion_policy.model.vq_bet.vqvae.vqvae import VqVae
...
class BehaviorTransformer(nn.Module):
    ...
    def forward(
        ...
        extra_obs_seq: Optional[torch.Tensor],
        goal_seq: Optional[torch.Tensor],
        action_seq: Optional[torch.Tensor],
    ) -> Tuple[Optional[torch.Tensor], Optional[torch.Tensor]]:
        return self._predict(obs_seq, extra_obs_seq, goal_seq, action_seq)
    ...
    def _predict(
        ...
        if self.visual_input:
            ...
            if obs_seq.ndim == 3:
                ...
                if extra_obs_seq is not None:
                    extra_obs_seq = einops.rearrange(extra_obs_seq, "N T C -> (N T) C")
                    obs_seq = torch.cat([obs_seq, extra_obs_seq], dim=-1)
