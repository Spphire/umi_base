...
from diffusion_policy.model.vq_bet.vector_quantize_pytorch.vector_quantize_pytorch import VectorQuantize
...
class ResidualVQ(nn.Module):
    ...
    def draw_logits_forward(self, encoding_logits):
        ...
        for q in range(encoding_logits.shape[1]):
            quantized += torch.matmul(encoding_logits[:, q], self.codebooks[q]).to(self.device)
        return quantized
