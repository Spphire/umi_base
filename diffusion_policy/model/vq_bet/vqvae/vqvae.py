import torch.nn
import einops
from diffusion_policy.model.common.distributions import DiagonalGaussianDistribution
from diffusion_policy.model.common.normalizer import LinearNormalizer
from diffusion_policy.model.common.shape_util import get_output_shape
from diffusion_policy.model.vq_bet.vector_quantize_pytorch.residual_vq import ResidualVQ
from diffusion_policy.model.vq_bet.vqvae.vqvae_utils import *
...
class VqVae:
    def __init__(
        ...
        if self.input_dim_h == 1:
            if self.use_conv_encoder:
                ...
                self.encoder = MLP(
                    input_dim=self.input_dim_w, output_dim=n_latent_dims, layer_num=mlp_layer_num
                ).to(self.device)

            output_shape = get_output_shape((1 * self.input_dim_h,), self.encoder)
            if len(output_shape) == 1:
                decoder_n_latent_dims = output_shape[0]
                self.downsampled_input_h = 1
            else:
                decoder_n_latent_dims = np.multiply(*output_shape)
                self.downsampled_input_h = output_shape[-1]
            ...
            if self.use_rnn_decoder:
                ...
                self.decoder = MLP(
                    input_dim=decoder_n_latent_dims, output_dim=self.input_dim_w, layer_num=mlp_layer_num
                ).to(self.device)
        else:
            if self.use_conv_encoder:
                self.encoder = EncoderCNN(
        ...
        if self.finetune_decoder:
            self.optim_params = list(self.decoder.parameters())
        else:
            self.optim_params = (
        ...
        self.vqvae_optimizer = torch.optim.Adam(
            self.optim_params, lr=self.vqvae_lr, weight_decay=0.0001
        )
    ...
    def compute_loss_and_metric(self, batch):
        ...
        if self.finetune_decoder:
            state_vq = batch["predicted_latent_action"]
        else:
            state_rep = self.encoder(state)
            if self.use_vq:
        ...
        return_dict = {
            ...
            "vqvae_recon_loss": vqvae_recon_loss.item(),
        }
