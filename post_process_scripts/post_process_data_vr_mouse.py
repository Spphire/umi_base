            ...
            if use_hand_masking and hand_segmentor is not None:
                ...
                if use_dino:
                    for img in obs_dict['left_eye_img']:
                        #img = cv2.LUT(img, table)
                        if resize_target is None:
                            img = resize_image(img, mode='crop')
                        else:
...
if __name__ == '__main__':
    # 示例使用
    input_dir = '/mnt/data/shenyibo/workspace/umi_base/.cache/targz_q3_stack_cup_fisheye'
    output_dir = '/mnt/data/shenyibo/workspace/q3_stack_cup_fisheye'
